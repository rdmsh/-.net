...
class TransformData
{
    ...
    const BAZON_BASE_URI = 'https://v1633364825.bazon.site/embed?search=';
    ...
    private function transformTitle($data, $type)
    {
        ...
        //full videos + videos
        $transformed['videos'] = array_merge($transformed['videos'], $this->getFullVideos($transformed));
    ...
    /**
     * @param array $transformedTmdbTitle
     * @return array
     */
    private function getFullVideos($transformedTmdbTitle)
    {
        $bazon_videos = [];
        $bazon_videos[] = [
            'name' => trim($transformedTmdbTitle['name']),
            'url' => self::BAZON_BASE_URI . trim($transformedTmdbTitle['name']) . " " . $transformedTmdbTitle['year'],
            'type' => Video::VIDEO_TYPE_EMBED,
            'source' => 'bazon',
            'category' => 'full'
        ];
